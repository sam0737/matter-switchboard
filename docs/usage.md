...
2. Start commissioning from the local CLI and enter the temporary setup code:
...
3. Switchboard commissions the strip onto its own Matter fabric and saves the
   ...
   changed.
4. Assign a unique slug if one was not supplied during commissioning:

   ```sh
   matter-switchboard device rename <current-slug> patio-strip
   ```
...
This asks the reachable device to remove the Switchboard fabric, then removes
the local pairing and inventory entry. Because the device removes the fabric
used for the active secure session, the session may close before a response is
received; the CLI verifies removal where possible and reports an indeterminate
result distinctly from success.
...
only local inventory and credentials. It warns that the fabric may remain on
the device and consume a fabric slot.
...
```sh
matter-switchboard key create --name automation-a --scope control --devices patio-strip
matter-switchboard key create --name dashboard --scope read
